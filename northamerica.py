...
NORTH_AMERICAN_LOCATIONS.update(
    {
...
CELL_STEP_DEGREES = 0.5
CELL_DATA_VERSION = "north-america-cell-landmask-v2"
...
MODE_CONTEXT = {
    "Rail + road": "major intercity rail corridors and nearby southern routes",
    ...
    "Air + remote access": "northern, remote, and low-access areas",
}
...
def is_north_american_land(lat: float, lon: float) -> bool:
...
def origin_location_options() -> dict[str, tuple[float, float]]:
    options: dict[str, tuple[float, float]] = {
        name.split(",")[0]: coordinates for name, coordinates in NORTH_AMERICAN_LOCATIONS.items()
    }
...
def nearest_known_place(lat: float, lon: float, threshold_km: float = 60) -> str:
    ...
    for name, coordinates in NORTH_AMERICAN_LOCATIONS.items():
...
def build_passage_cells(
    ...
    rows = []
    lat = NORTH_AMERICA_BOUNDS["min_lat"]

    while lat <= NORTH_AMERICA_BOUNDS["max_lat"]:
        lon = NORTH_AMERICA_BOUNDS["min_lon"]
        while lon <= NORTH_AMERICA_BOUNDS["max_lon"]:
            if is_north_american_land(lat, lon):
...
def build_city_estimates(origin: Place) -> pd.DataFrame:
    rows = []
    for name, (lat, lon) in NORTH_AMERICAN_LOCATIONS.items():
        estimate = estimate_travel(origin, lat, lon)
        rows.append(
            {
                "place": name,
...
def make_map(origin: Place, cells: pd.DataFrame, route_plan: RoutePlan | None = None) -> pdk.Deck:
    ...
    return pdk.Deck(
        ...
        initial_view_state=pdk.ViewState(latitude=45.0, longitude=-101.0, zoom=2.35, pitch=0),
        layers=layers,
        tooltip={
...
def render_sidebar() -> dict[str, object]:
    ...
    if custom_location:
        lat = st.sidebar.number_input(
            "Latitude",
            min_value=NORTH_AMERICA_BOUNDS["min_lat"],
            max_value=NORTH_AMERICA_BOUNDS["max_lat"],
            value=city_lat,
            format="%.6f",
        )
        lon = st.sidebar.number_input(
            "Longitude",
            min_value=NORTH_AMERICA_BOUNDS["min_lon"],
            max_value=NORTH_AMERICA_BOUNDS["max_lon"],
            value=city_lon,
            format="%.6f",
        )
...
def main() -> None:
    st.set_page_config(page_title="North America Isochronic Passage Chart", page_icon="NA", layout="wide")
    st.title("North America Isochronic Passage Chart")
